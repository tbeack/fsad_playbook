...
- [x] `CBP-082` Add `claude ultrareview [target]` non-interactive CLI subcommand (CI/scripts, `--json`, exit codes) to Cheat Sheet `/ultrareview` row; also note `/resume` accepts pasted PR URLs to find the originating session (v2.1.120 + v2.1.122) → [task-cbp-082.md](task-cbp-082.md)
- [x] `CBP-083` Add `${CLAUDE_EFFORT}` to the skill frontmatter String substitutions note (v2.1.120) → [task-cbp-083.md](task-cbp-083.md)
- [x] `CBP-084` Add `alwaysLoad` MCP server config option (skips tool-search deferral, all tools always available) to Power Usage Plugins collapsible (v2.1.121) → [task-cbp-084.md](task-cbp-084.md)
- [x] `CBP-085` Update `/plugin` Cheat Sheet row to mention `claude plugin prune` for removing orphaned auto-installed plugin dependencies (v2.1.121) → [task-cbp-085.md](task-cbp-085.md)
- [x] `CBP-086` Update Hooks section: PostToolUse `hookSpecificOutput.updatedToolOutput` now applies to all tools (was MCP-only); add note in Exit Codes section (v2.1.121) → [task-cbp-086.md](task-cbp-086.md)
