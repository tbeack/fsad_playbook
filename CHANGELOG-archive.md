...
Older version history for the FSAD Playbook. Split out of `CHANGELOG.md` on 2026-09-10 (CBP-544) to keep the active file small — everything before v4.1.0 lives here, verbatim, with no content lost.
...
- **CBP-341 — Resync follow-up: 5 of 12 shared skills had drifted since CBP-339's baseline.** A post-CBP-339 audit against the `tb_skills` v3.31.0 baseline found `add-task`, `estimate`, `prd`, `code-review-team`, and `sec-review-team` were still stale. `add-task`, `estimate`, and `prd` picked up small mechanical fixes. `code-review-team` and `sec-review-team` got the larger change: their hardcoded 5-pass consensus fan-out (`correctness-reviewer`/`performance-reviewer` in `code-review-team`; `auth-authz-auditor`/`input-validation-auditor` in `sec-review-team`) was replaced with a loop-until-dry mechanism — `MIN_PASSES=2`, `MAX_PASSES=8`, stopping the first round that introduces no new `root_issue` rather than always running a fixed count, so effort scales with how much variance a given diff/target actually produces. Both skills also had a re-review filter ported into their Step 4.5 — splitting ledger entries into `reconfirmed_known` (rediscovered and re-validated this run, suppressed from the report) and `carried_over_known` (not rediscovered, re-verified via a cheap grep against the ledger's stored `file`/`evidence_snippet` before being reported as "still open"), with explicit caveat categories for legacy ledger entries (predating `file`/`evidence_snippet` tracking) and critical/major findings that couldn't be mechanically re-verified. Both skills gained a new `schema/` directory (`finding.schema.json`, `coverage.schema.json`) formalizing the JSONL contract that Step 4's consolidation validates every specialist's output against. The playbook's Skills Library page has been updated to match: all 5 changed `SKILL.md` sources re-embedded, the `auth-authz-auditor`/`input-validation-auditor` specialist definition cards refreshed, every "5-pass" reference outside the historical changelog (visible diagram labels, `<desc>` accessibility text, cost/runtime estimate prose, skill card descriptions) rewritten to describe the loop-until-dry mechanism, and both the `code-review-team` and `sec-review-team` orchestration diagrams (dark and light variants) updated accordingly. `dist/` rebuilt to match.
