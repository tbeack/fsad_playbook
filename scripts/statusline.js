...
function runStatusline() {
  ...
  process.stdin.on("end", () => {
    ...
    try {
      ...
      // Session task label — written by tb:do-task / tb:add-task to <cwd>/.pmon-session-task
      // Format: "<color>:<label>" (e.g. "orange:PMON-061") or plain "<label>" (defaults to orange).
      let taskBadge = "";
      try {
        const labelFile = path.join(dir, ".pmon-session-task");
        if (fs.existsSync(labelFile)) {
          const raw = fs.readFileSync(labelFile, "utf8").trim();
          const colonIdx = raw.indexOf(":");
          const label = colonIdx > 0 ? raw.slice(colonIdx + 1) : raw;
          // Badge always renders green — the color prefix in .pmon-session-task is ignored.
          const colorCode = "\x1b[38;5;77m";
          if (label) taskBadge = `${colorCode}${label}\x1b[0m`;
        }
      } catch (e) {}

      // Output: model | project | task-id | context
      const dirname = path.basename(dir);
      const segs = [`\x1b[38;5;141m${model}\x1b[0m`, `\x1b[38;5;75m${dirname}\x1b[0m`];
      if (taskBadge) segs.push(taskBadge);
      process.stdout.write(segs.join(' │ ') + ctx + lastCmdSuffix);
    } catch (e) {
...
function renderStatusline(data) {
  ...
  if (middle) {
    return `\x1b[38;5;141m${model}\x1b[0m │ ${middle} │ \x1b[38;5;75m${dirname}\x1b[0m${lastCmdSuffix}`;
  }
  return `\x1b[38;5;141m${model}\x1b[0m │ \x1b[38;5;75m${dirname}\x1b[0m${lastCmdSuffix}`;
}
